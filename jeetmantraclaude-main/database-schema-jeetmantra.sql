...
CREATE TABLE jeetmantra_users (
    ...
    user_type ENUM('student', 'teacher', 'partner', 'admin', 'school') NOT NULL,
...
CREATE TABLE jeetmantra_user_profiles (
    ...
    user_type ENUM('student', 'teacher', 'partner', 'school') NOT NULL,
...
CREATE VIEW jeetmantra_user_summary AS
SELECT
    u.id,
    u.full_name,
    u.email,
...
LEFT JOIN jeetmantra_user_skills s ON u.id = s.user_id
LEFT JOIN jeetmantra_courses c ON u.id = c.teacher_id
LEFT JOIN jeetmantra_enrollments e ON u.id = e.student_id AND e.status = 'active'
GROUP BY u.id;

-- View: Course Summary with Enrollment Stats
CREATE VIEW jeetmantra_course_summary AS
SELECT
    c.id,
    c.title,
    c.teacher_id,
    ...
    c.created_at
FROM jeetmantra_courses c
LEFT JOIN jeetmantra_users u ON c.teacher_id = u.id
LEFT JOIN jeetmantra_enrollments e ON c.id = e.course_id
LEFT JOIN jeetmantra_feedback f ON c.id = f.course_id
GROUP BY c.id;
