...
'stage">\n    <div id="outWrap">\n      <canvas id="out" width="1280" height="720"></canvas>\n      <svg id="cornerSvg" viewBox="0 0 1280 720" preserveAspectRatio="none"></svg>\n      <div id="cropOverlay"></div>\n      <div id="wbSelBox" style="display:none;position:absolute;border:2px dashed #7c3aed;box-sizing:border-box;background:rgba(124,58,237,.06);cursor:move">\n        <div class="wb-rsz" data-dir="nw" style="position:absolute;left:-5px;top:-5px;width:10px;height:10px;background:#7c3aed;b',
'order-radius:2px;cursor:nw-resize"></div>\n        <div class="wb-rsz" data-dir="ne" style="position:absolute;right:-5px;top:-5px;width:10px;height:10px;background:#7c3aed;border-radius:2px;cursor:ne-resize"></div>\n        <div class="wb-rsz" data-dir="sw" style="position:absolute;left:-5px;bottom:-5px;width:10px;height:10px;background:#7c3aed;border-radius:2px;cursor:sw-resize"></div>\n        <div class="wb-rsz" data-dir="se" style="position:absolute;right:-5px;bottom:-5px;width:10px;height:1',
'0px;background:#7c3aed;border-radius:2px;cursor:se-resize"></div>\n      </div>\n    </div>\n    <!-- ══ S-Board Toolbar: 3 flat rows ══ -->\n    <div id="wbToolbar" style="flex-direction:column;margin-top:8px;gap:2px">\n\n      <!-- ROW 1: Draw · Text · Shapes · Ink color · Size · Undo -->\n      <div class="wb-row" id="wbRow1">\n        <button class="btn wb-tool on" data-tool="pen"    onclick="wbSetTool(\'pen\')"    title="Pen (P)"     style="padding:3px 9px;font-size:12px">✏️ Pen</button>\n ',
...
'n        <button id="wbFillToggle" onclick="wbToggleFillMode()" title="Outline / Filled" style="border:1px solid var(--bd);border-radius:5px;padding:2px 7px;font-size:10px;cursor:pointer;background:var(--panel2);color:#9098ac">Outline</button>\n        <div class="wb-sep"></div>\n        <!-- Dual color swatch -->\n        <div style="position:relative;width:32px;height:32px;flex-shrink:0;cursor:pointer" title="Primary / Secondary ink color">\n          <div id="wbColor2Swatch" onclick="wbActiva',
'teSlot(2)" style="position:absolute;right:0;bottom:0;width:20px;height:20px;border-radius:4px;background:#ffffff;border:1.5px solid #666"></div>\n          <div id="wbColor1Swatch" onclick="wbActivateSlot(1)" style="position:absolute;left:0;top:0;width:20px;height:20px;border-radius:4px;background:#1a1a25;border:2px solid #e8eaf0;box-shadow:0 0 0 1.5px #7c3aed"></div>\n        </div>\n        <button onclick="wbSwapColors()" title="Swap colors" style="border:1px solid var(--bd);border-radius:5px',
';padding:2px 5px;font-size:12px;cursor:pointer;background:var(--panel2);color:#e8eaf0">⇄</button>\n        <!-- Quick 8-color palette -->\n        <button onclick="wbSetColorSlot(\'#1a1a25\')" style="width:14px;height:14px;border-radius:3px;background:#1a1a25;border:1.5px solid #555;padding:0;cursor:pointer;flex-shrink:0" title="Black"></button>\n        <button onclick="wbSetColorSlot(\'#ffffff\')" style="width:14px;height:14px;border-radius:3px;background:#fff;border:1.5px solid #999;padding:0',
...
'1a25;border:1.5px solid #999;display:block"></span>\n          <span id="wbC2Mini" style="width:10px;height:10px;border-radius:2px;background:#fff;border:1px solid #666;display:block;margin-left:-3px;margin-top:5px;flex-shrink:0"></span>\n          <input type="color" id="wbColorPicker" value="#1a1a25" oninput="wbSetColorSlot(this.value)" style="position:absolute;opacity:0;width:100%;height:100%;cursor:pointer;border:none;padding:0;left:0;top:0">\n        </label>\n        <div class="wb-sep"></',
'div>\n        <!-- Size -->\n        <span id="wbSizeVal" style="font-size:11px;font-weight:700;min-width:18px;color:#9098ac;text-align:right">4</span>\n        <input id="wbSizeRange" type="range" min="1" max="60" value="4" oninput="wbSetSize(this.value);document.getElementById(\'wbSizeVal\').textContent=this.value" style="width:65px;accent-color:#7c3aed">\n        <div class="wb-sep"></div>\n        <button class="btn" onclick="wbUndo()"  title="Undo (Ctrl+Z)"  style="padding:3px 8px;font-size',
':12px">↩</button>\n        <button class="btn" onclick="wbRedo()"  title="Redo (Ctrl+Y)"  style="padding:3px 8px;font-size:12px">↪</button>\n        <button class="btn" onclick="wbClear()" title="Clear page"      style="padding:3px 8px;font-size:12px;color:#fc8181">🗑</button>\n      </div>\n\n      <!-- ROW 2: Select · Edit · Image · Doc ref · BG patterns -->\n      <div class="wb-row" id="wbRow2">\n        <button class="btn wb-tool" data-tool="select" onclick="wbSetTool(\'select\')" title="Se',
...
'ixed;z-index:350;background:#1d202b;border:1.5px solid #7c3aed;border-radius:14px;box-shadow:0 16px 56px rgba(0,0,0,.7);width:480px;max-height:92vh;overflow:hidden;flex-direction:column">\n      <!-- Header / drag handle -->\n      <div id="wbTextWindowHeader" style="display:flex;align-items:center;justify-content:space-between;padding:9px 14px;background:#7c3aed;border-radius:12px 12px 0 0;cursor:move;user-select:none;flex-shrink:0;gap:8px">\n        <span style="color:#fff;font-size:13px;font-',
...
' 9px;line-height:1;font-weight:700;flex-shrink:0">✕</button>\n      </div>\n      <!-- Formatting toolbar -->\n      <div style="display:flex;align-items:center;gap:2px;padding:5px 8px;background:#15171f;border-bottom:1px solid #2a2e3c;flex-wrap:wrap;flex-shrink:0">\n        <!-- Block types -->\n        <button onclick="wbRteBlock(\'h1\')" title="Heading 1" style="border:1px solid #2a2e3c;border-radius:5px;padding:2px 7px;font-size:11px;font-weight:900;cursor:pointer;background:#15171f;color:#e',
'8eaf0">H1</button>\n        <button onclick="wbRteBlock(\'h2\')" title="Heading 2" style="border:1px solid #2a2e3c;border-radius:5px;padding:2px 7px;font-size:11px;font-weight:900;cursor:pointer;background:#15171f;color:#e8eaf0">H2</button>\n        <button onclick="wbRteBlock(\'h3\')" title="Heading 3" style="border:1px solid #2a2e3c;border-radius:5px;padding:2px 7px;font-size:11px;font-weight:700;cursor:pointer;background:#15171f;color:#e8eaf0">H3</button>\n        <button onclick="wbRteBlock(',
'\'p\')"  title="Paragraph" style="border:1px solid #2a2e3c;border-radius:5px;padding:2px 7px;font-size:12px;cursor:pointer;background:#15171f;color:#e8eaf0">¶</button>\n        <div style="width:1px;height:16px;background:#2a2e3c;margin:0 2px;flex-shrink:0"></div>\n        <!-- Text formats -->\n        <button id="rteB" onclick="wbRteFmt(\'bold\')"          title="Bold (Ctrl+B)"      style="border:1px solid #2a2e3c;border-radius:5px;padding:2px 7px;font-size:13px;font-weight:900;cursor:pointer;',
'background:#15171f;color:#e8eaf0">B</button>\n        <button id="rteI" onclick="wbRteFmt(\'italic\')"        title="Italic (Ctrl+I)"    style="border:1px solid #2a2e3c;border-radius:5px;padding:2px 7px;font-style:italic;font-size:13px;cursor:pointer;background:#15171f;color:#e8eaf0">I</button>\n        <button id="rteU" onclick="wbRteFmt(\'underline\')"     title="Underline (Ctrl+U)" style="border:1px solid #2a2e3c;border-radius:5px;padding:2px 7px;text-decoration:underline;font-size:13px;curso',
'r:pointer;background:#15171f;color:#e8eaf0">U</button>\n        <button id="rteS" onclick="wbRteFmt(\'strikeThrough\')" title="Strikethrough"      style="border:1px solid #2a2e3c;border-radius:5px;padding:2px 7px;text-decoration:line-through;font-size:12px;cursor:pointer;background:#15171f;color:#e8eaf0">S</button>\n        <div style="width:1px;height:16px;background:#2a2e3c;margin:0 2px;flex-shrink:0"></div>\n        <!-- Text color -->\n        <label title="Text color" style="position:relati',
've;display:inline-flex;flex-direction:column;align-items:center;cursor:pointer;font-size:11px;font-weight:700;color:#e8eaf0;gap:1px;padding:2px 5px;border:1px solid #2a2e3c;border-radius:5px;background:#15171f">\n          A<span id="wbTextColorBar" style="display:block;width:14px;height:3px;border-radius:2px;background:#e8eaf0"></span>\n          <input type="color" id="wbTextColor" value="#e8eaf0" oninput="wbRteColor(this.value)" style="position:absolute;opacity:0;width:100%;height:100%;cursor',
':pointer;border:none;padding:0;left:0;top:0">\n        </label>\n        <!-- Highlight -->\n        <label title="Highlight" style="position:relative;display:inline-flex;align-items:center;cursor:pointer;padding:2px 5px;border:1px solid #2a2e3c;border-radius:5px;background:#15171f;color:#e8eaf0;font-size:13px">\n          🖍<input type="color" id="wbHighlightColor" value="#fef08a" oninput="wbRteHighlight(this.value)" style="position:absolute;opacity:0;width:100%;height:100%;cursor:pointer;borde',
'r:none;padding:0;left:0;top:0">\n        </label>\n        <div style="width:1px;height:16px;background:#2a2e3c;margin:0 2px;flex-shrink:0"></div>\n        <!-- Lists + blocks -->\n        <button onclick="wbRteInsert(\'bullet\')"   title="Bullet list"       style="border:1px solid #2a2e3c;border-radius:5px;padding:2px 6px;font-size:13px;cursor:pointer;background:#15171f;color:#e8eaf0">•</button>\n        <button onclick="wbRteInsert(\'numbered\')" title="Numbered list"     style="border:1px sol',
'id #2a2e3c;border-radius:5px;padding:2px 6px;font-size:11px;cursor:pointer;background:#15171f;color:#e8eaf0">1.</button>\n        <button onclick="wbRteInsert(\'quote\')"    title="Blockquote"        style="border:1px solid #2a2e3c;border-radius:5px;padding:2px 6px;font-size:13px;cursor:pointer;background:#15171f;color:#e8eaf0">❝</button>\n        <button onclick="wbRteInsert(\'link\')"     title="Insert link"       style="border:1px solid #2a2e3c;border-radius:5px;padding:2px 6px;font-size:12px',
';cursor:pointer;background:#15171f;color:#e8eaf0">🔗</button>\n        <button onclick="wbRteInsert(\'code\')"     title="Inline code"       style="border:1px solid #2a2e3c;border-radius:5px;padding:2px 5px;font-size:10px;cursor:pointer;background:#15171f;color:#e8eaf0">&lt;/&gt;</button>\n        <button onclick="wbRteInsert(\'table\')"    title="Table"             style="border:1px solid #2a2e3c;border-radius:5px;padding:2px 6px;font-size:13px;cursor:pointer;background:#15171f;color:#e8eaf0">▦',
'</button>\n        <button onclick="wbRteInsert(\'callout\')"  title="Callout / key tip" style="border:1px solid #2a2e3c;border-radius:5px;padding:2px 6px;font-size:13px;cursor:pointer;background:#15171f;color:#e8eaf0">💡</button>\n        <button onclick="wbRteInsert(\'divider\')"  title="Horizontal rule"   style="border:1px solid #2a2e3c;border-radius:5px;padding:2px 6px;font-size:13px;cursor:pointer;background:#15171f;color:#e8eaf0">—</button>\n        <button onclick="document.getElementById',
'(\'wbRteImgInput\').click()" title="Insert image" style="border:1px solid #2a2e3c;border-radius:5px;padding:2px 6px;font-size:13px;cursor:pointer;background:#15171f;color:#e8eaf0">🖼</button>\n        <input type="file" id="wbRteImgInput" accept="image/*" style="display:none" onchange="wbRteInsertImage(this)">\n        <button onclick="wbRteInsert(\'add\')" title="Add block" style="border:1.5px solid #7c3aed;border-radius:5px;padding:2px 7px;font-size:13px;cursor:pointer;background:rgba(124,58,2',
'37,.2);color:#c4b5fd;font-weight:700">＋</button>\n        <div style="flex:1"></div>\n        <!-- Align -->\n        <button id="rteLeft"   onclick="wbRteAlign(\'left\')"   title="Left"   style="border:1px solid #2a2e3c;border-radius:5px;padding:2px 5px;font-size:10px;cursor:pointer;background:#7c3aed;color:#fff">≡←</button>\n        <button id="rteCenter" onclick="wbRteAlign(\'center\')" title="Center" style="border:1px solid #2a2e3c;border-radius:5px;padding:2px 5px;font-size:10px;cursor:poin',
'ter;background:#15171f;color:#e8eaf0">≡↔</button>\n        <button id="rteRight"  onclick="wbRteAlign(\'right\')"  title="Right"  style="border:1px solid #2a2e3c;border-radius:5px;padding:2px 5px;font-size:10px;cursor:pointer;background:#15171f;color:#e8eaf0">≡→</button>\n      </div>\n      <!-- contenteditable editor -->\n      <div id="wbRteEditor" contenteditable="true" spellcheck="true"\n        data-placeholder="📌 Chapter 2 — Algebra Basics&#10;&#10;Variables, equations, solving…"\n      ',
'  style="flex:1;min-height:160px;max-height:320px;overflow-y:auto;padding:14px 16px;font-size:16px;line-height:1.75;background:#0b0d14;color:#e8eaf0;outline:none;font-family:system-ui,sans-serif">\n      </div>\n      <!-- Footer actions -->\n      <div style="display:flex;gap:8px;padding:9px 14px;background:#15171f;border-top:1px solid #2a2e3c;flex-shrink:0;align-items:center">\n        <button class="btn pri" onclick="wbStampRte()" style="flex:1;padding:8px;font-size:13px">✅ Stamp to Board</bu',
'tton>\n        <button onclick="wbRteInsert(\'bullet\')" style="border:1px solid #2a2e3c;border-radius:6px;padding:6px 10px;font-size:11px;cursor:pointer;background:#1d202b;color:#e8eaf0">• Bullets</button>\n        <button onclick="wbRteInsert(\'callout\')" style="border:1px solid #2a2e3c;border-radius:6px;padding:6px 10px;font-size:11px;cursor:pointer;background:#1d202b;color:#e8eaf0">💡 Note</button>\n        <button id="wbTextCancelBtn" class="btn" style="padding:8px 13px">✕</button>\n      ',
...
'="btn" onclick="document.getElementById(\'wbDocPanel\').style.display=\'none\'" style="padding:2px 7px;color:#e53e3e">✕</button>\n        </div>\n      </div>\n      <div style="overflow:auto;text-align:center;padding:6px;background:#1a1d2a;max-height:calc(80vh - 50px)">\n        <img id="wbDocImg" src="" alt="" style="max-width:100%;border-radius:6px;display:none">\n        <iframe id="wbDocPdf" src="" style="width:100%;height:500px;border:none;border-radius:6px;display:none"></iframe>\n       ',
' <div id="wbDocNoImg" style="padding:20px;color:#9098ac;font-size:13px">No document loaded yet.<br>Open an image or PDF from Insert → Doc ref.</div>\n        <button id="wbDocAddToBoard" onclick="wbDocAddToBoard()" style="display:none;margin-top:8px;border:1px solid #7c3aed;border-radius:8px;padding:5px 14px;font-size:12px;cursor:pointer;background:#1d202b;color:#e8eaf0">⬇ Add to board</button>\n      </div>\n    </div>\n\n    <!-- Save to course modal -->\n    <div id="wbSaveCourseModal" style=',
...
'n:column;align-items:center;justify-content:flex-start;gap:10px;padding:16px;overflow-y:auto">\n  <!-- Header row -->\n  <div style="display:flex;align-items:center;justify-content:space-between;width:min(88vw,960px);gap:12px">\n    <div>\n      <div style="font-size:15px;font-weight:800;color:#fff">🎬 Crop to Aspect Ratio</div>\n      <div id="arCropLabel" style="font-size:12px;color:#7c3aed;font-weight:700;margin-top:2px">16:9 · YouTube / Zoom</div>\n    </div>\n    <!-- AR quick-select chips ',
'-->\n    <div style="display:flex;gap:6px;flex-wrap:wrap;justify-content:flex-end">\n      <button onclick="arQuickSwitch(\'1920x1080\')" data-arq="1920x1080" style="border:1.5px solid #7c3aed;border-radius:20px;padding:4px 10px;font-size:11px;font-weight:700;cursor:pointer;background:#7c3aed;color:#fff">16:9</button>\n      <button onclick="arQuickSwitch(\'1080x1920\')" data-arq="1080x1920" style="border:1.5px solid #444;border-radius:20px;padding:4px 10px;font-size:11px;font-weight:700;cursor:',
'pointer;background:transparent;color:#ccc">9:16</button>\n      <button onclick="arQuickSwitch(\'1280x720\')"  data-arq="1280x720"  style="border:1.5px solid #444;border-radius:20px;padding:4px 10px;font-size:11px;font-weight:700;cursor:pointer;background:transparent;color:#ccc">HD</button>\n      <button onclick="arQuickSwitch(\'1080x1080\')" data-arq="1080x1080" style="border:1.5px solid #444;border-radius:20px;padding:4px 10px;font-size:11px;font-weight:700;cursor:pointer;background:transpare',
'nt;color:#ccc">1:1</button>\n      <button onclick="arQuickSwitch(\'960x720\')"   data-arq="960x720"   style="border:1.5px solid #444;border-radius:20px;padding:4px 10px;font-size:11px;font-weight:700;cursor:pointer;background:transparent;color:#ccc">4:3</button>\n    </div>\n  </div>\n  <div style="font-size:11px;color:#9098ac;width:min(88vw,960px)">Drag the box to reposition · drag corner handles to resize (aspect ratio is locked) · corner handles keep the crop ratio</div>\n  <!-- Preview + cr',
'op box -->\n  <div id="arCropWrap" style="position:relative;max-width:min(88vw,960px);width:min(88vw,960px);cursor:default;user-select:none;line-height:0;border-radius:8px;overflow:hidden">\n    <canvas id="arCropPreview" style="display:block;width:100%;height:auto;border-radius:8px;background:#111"></canvas>\n    <div id="arcDimT" style="position:absolute;top:0;left:0;right:0;background:rgba(0,0,0,.6);pointer-events:none"></div>\n    <div id="arcDimB" style="position:absolute;bottom:0;left:0;ri',
'ght:0;background:rgba(0,0,0,.6);pointer-events:none"></div>\n    <div id="arcDimL" style="position:absolute;background:rgba(0,0,0,.6);pointer-events:none"></div>\n    <div id="arcDimR" style="position:absolute;background:rgba(0,0,0,.6);pointer-events:none"></div>\n    <!-- Crop box -->\n    <div id="arCropBox" style="position:absolute;border:2px solid #7c3aed;box-sizing:border-box;cursor:move">\n      <!-- Rule-of-thirds grid lines -->\n      <div style="position:absolute;inset:0;pointer-events:',
...
'v>\n      <div class="arc-h" id="arc-bl" style="bottom:-7px;left:-7px;cursor:nesw-resize"></div>\n      <div class="arc-h" id="arc-br" style="bottom:-7px;right:-7px;cursor:nwse-resize"></div>\n    </div>\n  </div>\n  <!-- Platform hints -->\n  <div id="arPlatformHint" style="font-size:11px;color:#9098ac;width:min(88vw,960px);padding:6px 10px;background:rgba(124,58,237,.1);border:1px solid rgba(124,58,237,.3);border-radius:8px">\n    📺 <strong style="color:#e8eaf0">16:9</strong> — Best for: YouT',
...
'iles, read and show as text; for .doc/.docx, show download link\n  const r=new FileReader();\n  r.onload=ev=>{\n    const txt=ev.target.result;\n    if(noImg){ noImg.innerHTML=\'<pre style="text-align:left;white-space:pre-wrap;font-size:12px;color:#e8eaf0;max-height:460px;overflow:auto;padding:6px">\'+txt.replace(/</g,\'&lt;\')+\'</pre>\'; noImg.style.display=\'block\'; }\n    panel.style.display=\'block\';\n    if(!panel._dragged){ panel.style.right=\'16px\'; panel.style.top=\'70px\'; panel.sty',
...
'ly:monospace;font-size:.9em\';\n      try{ if(!r.collapsed){ r.surroundContents(code); } else { code.textContent=\'code\'; r.insertNode(code); } }catch(e){}\n    }\n    return;\n  }\n  if(type===\'table\'){\n    document.execCommand(\'insertHTML\',false,\'<table style="border-collapse:collapse;width:100%;margin:6px 0"><tr><td style="border:1px solid #3a3f55;padding:5px 9px">Cell</td><td style="border:1px solid #3a3f55;padding:5px 9px">Cell</td></tr><tr><td style="border:1px solid #3a3f55;padding',
':5px 9px">Cell</td><td style="border:1px solid #3a3f55;padding:5px 9px">Cell</td></tr></table><p></p>\');\n    return;\n  }\n  if(type===\'callout\'){\n    document.execCommand(\'insertHTML\',false,\'<div style="background:rgba(124,58,237,.15);border-left:4px solid #7c3aed;border-radius:6px;padding:9px 13px;margin:6px 0;color:#e8eaf0">💡 Key insight: write here</div><p></p>\');\n    return;\n  }\n  if(type===\'divider\'){\n    document.execCommand(\'insertHTML\',false,\'<hr style="border:none;bo',
...
function wbToggleFillMode(){
  wbFillMode=!wbFillMode;
  const btn=document.getElementById('wbFillToggle');
  if(btn){ btn.textContent=wbFillMode?'Filled':'Outline'; btn.style.color=wbFillMode?'#c4b5fd':'#9098ac'; btn.style.borderColor=wbFillMode?'#7c3aed':''; }
}
...
function wbRteInsert(type){
  ...
  if(type==='code'){
    ...
    if(sel&&sel.rangeCount){
      const r=sel.getRangeAt(0);
      const code=document.createElement('code');
      code.style.cssText='background:#2a2e3c;color:#f9a8d4;padding:1px 5px;border-radius:4px;font-family:monospace;font-size:.9em';
      try{ if(!r.collapsed){ r.surroundContents(code); } else { code.textContent='code'; r.insertNode(code); } }catch(e){}
    }
    return;
  }
  if(type==='table'){
    document.execCommand('insertHTML',false,'<table style="border-collapse:collapse;width:100%;margin:6px 0"><tr><td style="border:1px solid #3a3f55;padding:5px 9px">Cell</td><td style="border:1px solid #3a3f55;padding:5px 9px">Cell</td></tr><tr><td style="border:1px solid #3a3f55;padding:5px 9px">Cell</td><td style="border:1px solid #3a3f55;padding:5px 9px">Cell</td></tr></table><p></p>');
    return;
  }
  if(type==='callout'){
    document.execCommand('insertHTML',false,'<div style="background:rgba(124,58,237,.15);border-left:4px solid #7c3aed;border-radius:6px;padding:9px 13px;margin:6px 0;color:#e8eaf0">💡 Key insight: write here</div><p></p>');
  ...
  if(type==='divider'){
    document.execCommand('insertHTML',false,'<hr style="border:none;border-top:2px solid #3a3f55;margin:10px 0"><p></p>');
    return;
  }
  if(type==='add'){
    document.execCommand('insertHTML',false,'<p><br></p>');
